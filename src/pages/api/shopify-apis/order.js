...
export default async function handler(req, res) {
  ...
  const fontTwo = fs.readFileSync(
    path.join(__dirname + "../../../../utils/fonts/OLDENGL.TTF")
  ...
  const titlePackPrinted = async (propObject) => {
    try {
      if (propObject.p_8727183196433._Title2) {
        console.log("herrrrrrrrrrrrreeeeee");
        var page = pdfDoc.addPage([595, 842]);
  ...
  if (email && req.body.line_items.length > 0) {
    ...
    req.body.line_items.map((item) => {
      ...
      console.log(
        pProperties["p_" + item.product_id].properties,
        "========================gyujhbmjh"
      );
    ...
    // console.log(
    //   pProperties["p_8727183196433"].properties,
    //   "pProperties ================="
    // );
    ...
    if (
      ...
      if (pProperties[`p_8727183196433`].properties) {
        console.log(pProperties[`p_8727183196433`].properties, "checking");
        namesArrayTitlePacks = pProperties[`p_8727183196433`].properties.map(
          (propItem, index) => propItem.name
        );
        console.log(namesArrayTitlePacks, "check again");
        for (const obj of pProperties[`p_8727183196433`].properties) {
          // console.log(obj, "objobj");
          resultObjectTitlePack[obj.name] = obj.value;
        }

        // for (const obj of pProperties[`p_${titlePackId}`].variant_title) {
        //   console.log(obj, "objobj");
        //   resultObjectTitlePack[obj.variant_title] = obj.variant_title;
        // }
      }
      // console.log(resultObjectTitlePack, "resultObjectTitlePack");
      // console.log(
      //   namesArrayTitlePacks,
      //   "namesArrayTitlePacks namesArrayTitlePacks"
      // );
      if (!namesArrayTitlePacks.includes("_Title2")) {
        ...
        // if (resultObjectTitlePack.variant_title == "Digital Download") {
        titlePack(propertiesObj);
        // } else {
        //   titlePackPrinted(propertiesObj);
        // }
